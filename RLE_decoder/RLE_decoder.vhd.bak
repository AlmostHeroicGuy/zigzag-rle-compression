    ...
    begin
        if reset = '1' then
            ...
            case state is
                ...
                -- idle: Wait for start signal to begin decoding
                when idle =>
                    done             <= '0';
                    rle_read_count   <= 0;
                    rle_read_index   <= 0;
                    ...
                    if start = '1' then
                        -- Capture first RLE pair immediately
                ...
                -- fill_rle_buffer: Read all RLE pairs from input
                when fill_rle_buffer =>
                    ...
                    if rle_read_count = to_integer(reduced_length) - 1 then
                        -- All RLE pairs captured, prepare for expansion
                        rle_read_index   <= 0;
                        zigzag_write_idx <= 0;
                ...
                -- expand_rle: Expand RLE pairs and fill matrix in zigzag order
                when expand_rle =>
                    if zigzag_write_idx < 64 then
                        -- Write current symbol to matrix at zigzag position
                        mem(zigzag_order(zigzag_write_idx)) <= current_symbol;
                        zigzag_write_idx <= zigzag_write_idx + 1;
                        expand_count <= expand_count + 1;
                        
                        -- Check if current RLE pair is exhausted
                        if expand_count = current_count - 1 then
                            -- Move to next RLE pair
                        ...
                        -- All 64 matrix positions filled
                        output_index <= 0;
                        state        <= output_data;
                    end if;
                
                
                -- output_data: Output matrix in row-major order
                when output_data =>
                    if output_index = 0 then
                        ...
                        -- Keep done high, output last element
                        done <= '1';
                    end if;
